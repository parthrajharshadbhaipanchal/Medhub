...
export default function SinglePharmacistRequest() {
  // state which is used to store pharmacist details
  ...
  // fetch data of unverified pharmacist
  const fetchData = async () => {
    getSingleUnverifiedPharmacist(reqData)
      .then((res) => {
        const data = res.pharmacistData;
        const rows = [
          createData("First Name", data["firstName"]),
          createData("Last Name", data["lastName"]),
          createData("Pharmacy Name", data["pharmacyName"]),
          ...
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <Paper sx={{ p: 2, display: "flex", flexDirection: "column" }}>
                ...
                <TableGrid rows={pharmacistDetails} />
                {isApproved ? (
